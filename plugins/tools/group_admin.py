...
async def handle(bot, event, cmd, args):
    ...
    if cmd in ('禁言', '解禁'):
        if not args:
            await bot.send(f'用法: ${cmd} @某人 <时长>（时长示例: 10分钟 / 1小时 / 30秒）', group_id=group_id)
            return True
        ...
        if cmd == '禁言':
            ...
            if duration is None:
                ...
                return True
            result = await bot.diy.set_group_ban(group_id=group_id, user_id=target, duration=duration)
        else:
            result = await bot.diy.set_group_ban(group_id=group_id, user_id=target, duration=0)
        if result and result.get('status') == 'ok':
            await bot.send(f'✅ 已{"禁言" if cmd == "禁言" else "解除禁言"} {target}'
                           + (f'（{args.split(" ", 1)[0]}）' if cmd == '禁言' else ''), group_id=group_id)
        else:
            await bot.send(f'操作失败: {result.get("msg", "未知错误") if result else "无响应"}', group_id=group_id)
        return True
    ...
    if cmd == '公告':
        ...
        if result and result.get('status') == 'ok':
            ...
            await bot.send(f'发布失败: {result.get("msg", "未知错误") if result else "无响应"}', group_id=group_id)
        return True
