...
def list_quotes(bot, group_id, category="", limit=20):
    ...
    return [
        {"id": r[0], "category": r[1], "content": r[2], "user_id": r[3], "ts": r[4]} for r in rows
    ]
...
async def _extract_reply_content(bot, event):
    ...
    for seg in getattr(event, "message", []) or []:
        if seg.type == "reply":
            ...
            text = "".join(
                s.get("data", {}).get("text", "") for s in quoted if s.get("type") == "text"
            )
            if text.strip():
                return text.strip()
...
async def on_message(event, bot):
    ...
    if cmd == "收录":
        if not args:
            ...
            if not content:
                await bot.send(
                    "用法: $收录 [分类] <内容>；或引用（回复）一条消息后发 $收录，收录被引用的内容",
                    group_id=group_id,
                )
                return True
            category = ""
    ...
    if cmd == "语录":
        ...
        if not q:
            await bot.send(
                f"还没有{category + '分类的' if category else ''}语录，用 $收录 添加吧",
                group_id=group_id,
            )
            return True
    ...
    if cmd == "语录列表":
        ...
        if not quotes:
            await bot.send(
                f"还没有{category + '分类的' if category else ''}语录", group_id=group_id
            )
            return True
        lines = [
            f"#{q['id']} [{q['category'] or '默认'}] {q['content'][:50]}（{q['user_id']} {fmt_ts(q['ts'])}）"
            for q in quotes
        ]
