...
def load_webui_config():
    try:
        with open(WEBUI_JSON_PATH, encoding="utf-8") as f:
            return json.load(f)
    except Exception:
        return {}
...
def require_login():
    ...
    if (
        ...
        or request.path == "/api/auth_status"
    ):
        return None
...
def login_logs():
    ...
    lines = []
    try:
        with open(LOGIN_LOG_PATH, encoding="utf-8") as f:
...
def change_password():
    ...
    return jsonify(
        {"message": "用户名与密码已修改" if new_username is not None else "密码修改成功"}
    )
...
def get_config(config_type):
    try:
        ...
        app.logger.error(f"Error loading config {config_type}: {e!s}")
        return jsonify({"error": str(e)}), 500
...
def update_config(config_type):
    try:
        ...
        app.logger.error(f"Error updating config {config_type}: {e!s}")
        return jsonify({"error": str(e)}), 500
...
def get_plugins():
    try:
        ...
        app.logger.error(f"Error getting plugins: {e!s}")
        return jsonify({"error": str(e)}), 500
...
def get_plugin_details(plugin_name):
    ...
    try:
        ...
        app.logger.error(f"Error getting plugin details for {plugin_name}: {e!s}")
        return jsonify({"error": str(e)}), 500
...
def toggle_plugin(plugin_name):
    ...
    try:
        ...
        app.logger.error(f"Error toggling plugin {plugin_name}: {e!s}")
        return jsonify({"error": str(e)}), 500
...
def uninstall_plugin(plugin_name):
    ...
    try:
        ...
        app.logger.error(f"Error uninstalling plugin {plugin_name}: {e!s}")
        return jsonify({"error": str(e)}), 500
...
def get_available_plugins():
    try:
        ...
        app.logger.error(f"Error getting available plugins: {e!s}")
        return jsonify({"error": str(e)}), 500
...
def get_plugin_detail(plugin_id: str):
    ...
    try:
        ...
        app.logger.error(f"Error getting plugin detail {plugin_id}: {e!s}")
        return jsonify({"error": str(e)}), 500
...
def _process_plugin_structure(plugin_name, extracted_plugin_root_path, log_callback):
    ...
    if (
        len(contents_at_root_path) == 1
        and os.path.isdir(os.path.join(extracted_plugin_root_path, contents_at_root_path[0]))
        and contents_at_root_path[0] == plugin_name
    ):
    ...
    if not os.path.exists(setup_path):
        if os.path.exists(legacy_entry):
            ...
            log_callback(
                f"已将旧式入口 '{plugin_name}.py' 重命名为 'setup.py'（目录插件的入口约定）。"
            )
        else:
            # 没有 setup.py 也没有同名 .py，退而求其次找唯一的顶层 .py 当入口
            py_files = [
                f
                for f in os.listdir(extracted_plugin_root_path)
            ...
            if len(py_files) == 1:
                ...
                log_callback(
                    "⚠️ 未找到 setup.py，插件可能无法被加载（目录插件入口必须叫 setup.py）。"
                )
    ...
    def generate_install_logs():
        ...
        try:
            if not plugin_url or not plugin_name or not plugin_path_in_repo:
                yield "data: Error: Missing plugin URL, name, or path\n\n"
                ...
                for chunk in response.iter_content(chunk_size=8192):
                    if chunk:
                        ...
                        if total_size > 0:
                            ...
                            yield from log_progress(
                                f"下载中: {downloaded_size}/{total_size} ({percent:.2f}%)"
                            )
                        else:
            ...
            if not extracted_repo_root:
                dirs_in_temp = [
                    d
                    for d in os.listdir(temp_extract_root)
                    if os.path.isdir(os.path.join(temp_extract_root, d))
                ]
            ...
            if not os.path.exists(source_plugin_content_dir):
                raise Exception(
                    f"Plugin content directory '{source_plugin_content_dir}' not found in the extracted repository."
                )
            ...
            yield from log_progress(
                f"插件 '{plugin_name}' 内容已移动到临时安装位置 '{final_plugin_target_dir}'。"
            )
            ...
            if os.path.exists(final_plugin_target_dir) and os.path.isdir(final_plugin_target_dir):
                ...
                if os.path.exists(requirements_path):
                    ...
                    if process.returncode != 0:
                        ...
                        yield "data: Error: 插件已安装，但依赖安装失败。详情请查看日志。\n\n"
                        yield "data: INSTALL_FAILED\n\n"
                        return
                    ...
                    yield from log_progress(
                        f"插件 {plugin_name} (目录插件) 没有找到 requirements.txt，跳过依赖安装。"
                    )
            else:
                yield from log_progress(
                    f"插件 {plugin_name} (单文件插件) 没有找到 requirements.txt，跳过依赖安装。"
                )

            yield from log_progress(f"插件 {plugin_name} 安装成功。")
            yield "data: INSTALL_SUCCESS\n\n"
            ...
            error_msg = f"安装插件 {plugin_name} 时发生错误: {e!s}"
            app.logger.error(error_msg)
            yield f"data: Error: {error_msg}\n\n"
            yield "data: INSTALL_FAILED\n\n"
        finally:
