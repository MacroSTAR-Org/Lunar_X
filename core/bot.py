...
class LunarBot:
    def __init__(self, config: dict[str, Any], main_loop: asyncio.AbstractEventLoop):
        ...
        if self.protocol_name == "milky":
            ...
            self.connection = WebSocketConnection(
                config["ws_server"], config.get("token"), max_retries=5
            )
    ...
    def plugin_dir(self, name: str) -> str | None:
        ...
        try:
            ...
            if response_data:
                if response_data.get("status") == "ok":
                    logger.success(
                        f"调用自定义API成功: {action}, 参数: {params}, 响应: {response_data}"
                    )
                    return {
                        "status": "ok",
                    ...
                    logger.error(
                        f"调用自定义API失败 (服务器响应错误): {action}, 错误信息: {error_msg}, 响应: {response_data}"
                    )
    ...
    def _check_permission(self, user_id: int | None, required_level: str = "manager") -> bool:
        ...
        if required_level == "super":
            ...
            return user_id in self.config.get("manager_users", []) or user_id in self.config.get(
                "super_users", []
            )
    ...
    def _build_help_message(self):
        ...
        for name, plugin_info in current_plugins_info["enabled_on_disk"].items():
            if (
                "error" not in plugin_info
                and plugin_info.get("help") != "未加载 (可能在上次加载时被跳过)"
            ):
    ...
    def _build_plugin_detail_help(self, plugin_name: str):
        ...
        for name, plugin in self.plugin_manager.plugins.items():
            if (
                name.lower() == plugin_name.lower()
                or plugin["trigger"].lower() == plugin_name.lower()
            ):
                found_plugin = plugin
                break
        ...
        if found_plugin:
            ...
            detail_help += "✅ 状态: 已启用并加载\n"
            detail_help += f"📁 类型: {found_plugin['type']}\n"
            return detail_help

        all_plugins_status = self.plugin_manager.get_plugin_list()

        for name, _info in all_plugins_status["disabled_on_disk"].items():
            if name.lower() == plugin_name.lower():
                return (
                    f"插件 {plugin_name} 已禁用 (文件/目录名为 d_{name}.py 或 d_{name})，未加载。"
                )
        ...
        for name, info in all_plugins_status["enabled_on_disk"].items():
            if name.lower() == plugin_name.lower() and "error" in info:
                ...
                name.lower() == plugin_name.lower()
                and info.get("help") == "未加载 (可能在上次加载时被跳过)"
            ):
    ...
    async def run(self):
        ...
        self.plugins = self.plugin_manager.plugins
        if self.config.get("auto_reload_plugins") is True:
    ...
    async def _handle_restart_info(self):
        ...
        try:
            with open(restart_info_file, encoding="utf-8") as f:
    ...
    async def _listen_events(self):
        ...
        try:
            async for event_data in self.connection.listen():
                logger.debug(
                    f"收到原始事件: {json.dumps(event_data, ensure_ascii=False, indent=2)}"
                )
    ...
    def _extract_text_from_message(self, message_data: list[dict | BaseSegment]) -> str:
        text_parts = []
    ...
    async def send_message_segments(
        ...
        try:
            ...
            if group_id:
                ...
                response_data = await self.protocol.send_message(
                    message_segments, group_id=group_id
                )
            elif user_id:
            ...
            if response_data and response_data.get("status") == "ok":
                ...
                error_msg = (
                    response_data.get("message", "未收到服务器确认或服务器返回错误")
                    if response_data
                    else "未收到服务器确认或服务器返回错误"
                )
            ...
            logger.error(
                f"发送消息超时，未收到服务器确认。目标: {'群' if group_id else '用户'} {group_id if group_id else user_id}"
            )
            return False
        except Exception as e:
    ...
    async def del_message(
        ...
        try:
            result = await self.protocol.delete_message(
                message_id, user_id=user_id, group_id=group_id
            )
    ...
    async def send(
        self,
        message: str | dict | list[dict | BaseSegment] | BaseSegment,
        user_id: int | None = None,
        group_id: int | None = None,
    ) -> bool:
