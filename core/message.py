...
class ImageSegment(BaseSegment):
    def __init__(self, file: str, cache: bool = True, proxy: bool = True, timeout: int = 30):
        super().__init__(
            "image", {"file": file, "cache": cache, "proxy": proxy, "timeout": timeout}
        )
...
class VideoSegment(BaseSegment):
    def __init__(self, file: str, cache: bool = True, proxy: bool = True, timeout: int = 30):
        super().__init__(
            "video", {"file": file, "cache": cache, "proxy": proxy, "timeout": timeout}
        )
...
class FileSegment(BaseSegment):
    def __init__(self, file_id: str, file_name: str = "", file_size: int = 0):
        super().__init__(
            "file", {"file_id": file_id, "file_name": file_name, "file_size": file_size}
        )
...
class ForwardSegment(BaseSegment):
    def __init__(self, nodes: list[ForwardNodeSegment]):
        super().__init__(
            "forward",
            {"id": str(int(time_module.time())), "nodes": [node.to_dict() for node in nodes]},
        )
...
class MessageBuilder:
    ...
    def image(
        self, file: str, cache: bool = True, proxy: bool = True, timeout: int = 30
    ) -> ImageSegment:
        if not os.path.isabs(file) and not (
            file.startswith("http://")
            or file.startswith("https://")
            or file.startswith("base64://")
        ):
    ...
    def reply(self, message_id: int | str, quoted_segments: list | None = None) -> ReplySegment:
    ...
    def combine(self, *message_segments: BaseSegment | str | dict) -> list[BaseSegment]:
        combined = []
    ...
    def _parse_dict_to_segment(self, segment_dict: dict[str, Any]) -> BaseSegment | None:
        ...
        if seg_type == "text":
            ...
            return self.image(
                seg_data.get("file", ""),
                seg_data.get("cache", True),
                seg_data.get("proxy", True),
                seg_data.get("timeout", 30),
            )
        elif seg_type == "at":
            ...
            return self.file(
                seg_data.get("file_id", ""),
                seg_data.get("file_name", ""),
                seg_data.get("file_size", 0),
            )
        elif seg_type == "reply":
            return self.reply(seg_data.get("id"), seg_data.get("quoted_segments"))
        return None

    def gen_message(self, data: dict | list) -> list[BaseSegment]:
        ...
        for segment in message_data:
            if isinstance(segment, BaseSegment):
                ...
                if parsed_segment:
                    ...
                    logger.warning(
                        f"无法将字典解析为消息段对象，跳过: {segment.get('type', '未知类型')}"
                    )
...
class ReplyUtils:
    ...
    @staticmethod
    def extract_reply_id_from_cq_code(text: str) -> str | None:
        patterns = [
            r"[CQ:reply,id=(\d+)]",
            r"[reply id=(\d+)]",
            r'[CQ:reply,id="(\d+)"]',
    ...
    def parse_cq_code(text: str) -> list[dict]:
        ...
        if reply_id:
            ...
            text = re.sub(
                r'[CQ:reply,id=(\d+)]|[reply id=(\d+)]|[CQ:reply,id="(\d+)"]|[reply id="(\d+)"]',
                "",
                text,
            )
    ...
    def extract_reply_id(self, event_message: list[BaseSegment]) -> str | None:
