...
class MilkyAdapter:
    ...
    def _internal_to_milky_segments(self, segments: list) -> list[dict]:
        ...
        for s in segments:
            ...
            if stype == "text":
                ...
                result.append(
                    {"type": "face", "data": {"face_id": data.get("id", 0), "is_large": False}}
                )
            elif stype == "reply":
        ...
        try:
            if group_id:
                action, params = (
                    ...
                    {"message_scene": "group", "peer_id": group_id, "message_seq": message_id},
                )
            elif user_id:
                action, params = (
                    "recall_private_message",
                    {"message_scene": "friend", "peer_id": user_id, "message_seq": message_id},
                )
            else:
                logger.error("Milky 撤回消息需要 user_id 或 group_id（消息场景）")
                return False
            result = await self.bot.connection.send(
                {"action": action, "params": params}, wait_for_response=True
            )
            return result
    ...
    async def get_forward_message(self, message_id: str) -> dict:
        try:
            ...
            return {
                "status": "failed",
                "msg": response.get("message", "获取失败") if response else "未收到响应",
            }
        except Exception as e:
    ...
    async def call_api(self, action: str, params: dict[str, Any]) -> dict[str, Any]:
        ...
        if action in TRANSFORM_ACTIONS:
            ...
            logger.warning(
                f"action '{action}' 未在映射表中，按原名透传给 Milky 协议端（插件可能需要迁移）"
            )

        # 发送类 action 的 message 参数做段序列化
        if milky_action in ("send_group_message", "send_private_message") and isinstance(
            milky_params, dict
        ):
            milky_params = dict(milky_params)
            milky_params["message"] = self._internal_to_milky_segments(
                milky_params.get("message", [])
            )

        response = await self.bot.connection.send(
            {"action": milky_action, "params": milky_params}, wait_for_response=True
        )
        ...
        for s in segments:
            ...
            if stype == "text":
                ...
                result.append(
                    {
                        "type": "image",
                        "data": {"file": data.get("temp_url") or data.get("resource_id", "")},
                    }
                )
            elif stype == "record":
                result.append(
    ...
    def _to_onebot_like(self, raw: dict[str, Any]) -> dict[str, Any]:
        ...
        if event_type == "message_receive":
            ...
            text = " ".join(
                s.get("data", {}).get("text", "") for s in segs if s.get("type") == "text"
            )
            message = {
                **base,
                "post_type": "message",
        ...
        if event_type == "group_whole_mute":
            return {
                **base,
                ...
                "operator_id": d.get("operator_id"),
                "duration": 0,
                "sub_type": "ban" if d.get("is_mute") else "lift_ban",
            }
